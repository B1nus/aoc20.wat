(component
  (core module $main
    ...
    (import "std" "u64.format" (func $"u64.format" (param i64) (result i32 i32)))
    ...
    (func $main
      ...
      (local $len i32)
      (call $input)
      (local.set $len (i32.sub (i32.const 1)))
      (local.set $ptr)
      ...
      (local.set $height (i32.div_u (local.get $len) (local.get $width)))
      (local.set $count (i64.const 1))
      (local.set $count (i64.mul (local.get $count) (i64.extend_i32_u (call $trees
        (local.get $ptr)
        (local.get $len)
        (local.get $width)
        (local.get $height)
      ...
      (call $"u64.format" (local.get $count))
      (call $"list u8.add" (i32.const 10))
      (call $output)
    )
    (func $trees (param $ptr i32) (param $len i32) (param $width i32) (param $height i32) (param $dx i32) (param $dy i32) (result i32)
      (local $count i32)
      (local $x i32)
      (local $y i32)
      (local $index i32)
      (loop $move
        ...
        (br_if $move (i32.lt_u (local.get $y) (local.get $height)))
      )
  ...
  (core module $std
    ...
    (memory (export "mem") 1)
    (func $output (export "output") (param $ptr i32) (param $len i32)
      (loop $write
        (call $"[method]output-stream.blocking-write-and-flush"
          (call $get-stdout)
          (local.get $ptr)
          (if (result i32) (i32.gt_u (local.get $len) (i32.const 0x1000))
            (then (i32.const 0x1000))
            (else (local.get $len))
          )
          (call $realloc (i32.const 0) (i32.const 8) (i32.const 4) (i32.const 0))
        )
        (local.set $len (i32.sub (local.get $len) (i32.const 0x1000)))
        (local.set $ptr (i32.add (local.get $ptr) (i32.const 0x1000)))
        (br_if $write (i32.gt_s (local.get $len) (i32.const 0)))
      )
    )
    (func $"list u8.join" (export "list u8.join") (param $ptr i32) (param $len i32) (param $other_ptr i32) (param $other_len i32) (result i32 i32)
      (local $new_ptr i32)
      (local $new_len i32)
    ...
    (func $"list u8.add" (export "list u8.add") (param $ptr i32) (param $len i32) (param $byte i32) (result i32 i32)
      (local $mem i32)
    ...
    (func $"u64.format" (export "u64.format") (param $number i64) (result i32 i32)
      (local $len i32)
      (local $num i64)
      (local $ptr i32)
      (local $i i32)
      ...
      (loop $len
        ...
        (br_if $len (i64.gt_u (local.get $num) (i64.const 0)))
      )

      (local.set $ptr (call $realloc (i32.const 0) (local.get $len) (i32.const 4) (i32.const 0)))
      (loop $store
        (local.set $i (i32.add (local.get $i) (i32.const 1)))
        (i32.store8 (i32.sub (i32.add (local.get $ptr) (local.get $len)) (local.get $i)) (i32.add (i32.wrap_i64 (i64.rem_u (local.get $number) (i64.const 10))) (i32.const 48)))
        (local.set $number (i64.div_u (local.get $number) (i64.const 10)))
        (br_if $store (i64.gt_u (local.get $number) (i64.const 0)))
      )
    
      (local.get $ptr)
      (local.get $len)
    )
    (func $"u32.format" (export "u32.format") (param $number i32) (result i32 i32)
      (local $len i32)
      (local $num i32)
      (local $ptr i32)
      (local $i i32)
    ...
    (func (export "input") (result i32 i32)
      ...
      (local $ptr i32)
      (local $len i32)
      (local.set $vecPtr (call $alloc (i32.const 12) (i32.const 4)))
      (loop $read
        ...
        (local.set $curPtr (i32.load offset=4 (local.get $vecPtr)))
        (local.set $curLen (i32.load offset=8 (local.get $vecPtr)))
        (call $alloc (local.get $curLen) (i32.const 4))
        (drop)
        (if (i32.eq (i32.load (local.get $vecPtr)) (i32.const 0))
          (then
            (local.set $len (i32.add (local.get $len) (local.get $curLen)))
            (br $read)
          )
        )
      )
      (i32.add (local.get $vecPtr) (i32.const 12))
      (local.get $len)
    )
    (func $alloc (export "alloc") (param $bytes i32) (param $alignment i32) (result i32)
      (call $realloc (i32.const 0) (local.get $bytes) (local.get $alignment) (i32.const 0)) 
    )
  )
  ...
  (core instance $main (instantiate $main
    ...
    (with "std" (instance
      ...
      (export "u64.format" (func $std "u64.format"))
